      ...
      .then(data => {
        if (data && data.events && Array.isArray(data.events) && data.events.length > 0) {
          setEvents(data.events);
           ...
           // If we get an empty list or error from server, trigger the fallback
           throw new Error("No events from server");
        }
      })
      .catch(err => {
        console.error("World Cup fetch failed, using fallback:", err);
        const fallback: any[] = [
              // Group A
