    ...
    try {
      ...
      try {
         ...
         if (sofaRes.ok) {
            ...
            if (sofaData && sofaData.events && Array.isArray(sofaData.events) && sofaData.events.length > 0) {
               ...
               setAllLiveMatches(prev => {
                  const combined = [...prev, ...liveOnly];
                  // Unique by ID
                  return Array.from(new Map(combined.map(item => [item.id, item])).values());
               });
               if (liveOnly.length > 0) setLiveMatch(liveOnly[0]);
            }
         }
      } catch(e) {
         console.warn("Live events API failed:", e);
      }
